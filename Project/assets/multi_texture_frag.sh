...
uniform sampler2D usTexture8;
uniform sampler2D usTexture9;
...
void main(){
    ...
    }else if(zp > 8.9 && zp < 9.1) {
        finalColor = texture2D(usTexture8, vTextureCoord);
    }else if(zp > 9.9 && zp < 10.1) {
        finalColor = texture2D(usTexture9, vTextureCoord);
    }else {
        finalColor = texture2D(usTexture0, vTextureCoord);
    }

    if(uRender_tag > 0.9 && zp > 3.9 && finalColor.a < 0.8) {
        finalColor.a = 0.0;
    }
